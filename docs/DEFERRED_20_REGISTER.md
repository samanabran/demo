...
| **D-08** | **PDPL Executive Regulations status** | **TENANT_CONFIG / VENDOR (regulatory)** | **HIGH — pre-go-live** | The Executive Regulations are not yet published (per Chambers 2026 commentary, 10 March 2026). The constant is `UNVERIFIED` with `valid_from=null`. Re-verification is a named pre-go-live task, **not a test**. | The Executive Regulations publish. |
...
| D-11 | G7 / G15 / G16 conversion from `CAPABILITY_PROVEN` to `CLOSED` | VENDOR | MEDIUM | The primitives are built and unit-tested. Conversion requires the first consumer wired (per the CAPABILITY_PROVEN definition). | The first consumer module installed. |
...
| D-14 | G29 bi-annual CO/MLRO report template + scheduler | VENDOR (template) / TENANT_DECISION (content) | MEDIUM | The recurrence and the template are ours; the content is the CO/MLRO's. | Wave 3. |
| D-15 | Cross-border safeguard field on the screening adapter | VENDOR (field) / TENANT_CONFIG (value) | MEDIUM | The safeguard enum is in the rules pack. The screening adapter's attestation field is not yet wired to the readiness gate. | Wave 2 item 5. |
...
| D-17 | `ir.rule` estate-wide tenant-isolation audit | VENDOR (architecture) | HIGH | Per the `check_company` verdict, the audit has never verified this at the system level. The first run of the test will likely reveal gaps. | The audit run, the gap list, and the per-model `ir.rule` records. |
